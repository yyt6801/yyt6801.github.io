...
#### 若本地仓库主分支与远程仓库的分支不一致时  
* 方法一：把本地 master 推到远程 main：  
    例如：本地分支master，远程空仓库默认分支main，想把本地 master 推上去，变成远程的 main，需要执行如下命令：  
    ```Bash
...
例如：想要忽略的文件已经被Git跟踪，那么仅仅添加到.gitignore是不够的，需要先从Git索引中移除这个文件：  
```Bash
...
> URL: https://blog.yyt6801.top/posts/git-common-operations/  
